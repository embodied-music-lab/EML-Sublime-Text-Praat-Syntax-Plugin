...
- **Automatic Praat discovery.** If Praat isn't at the standard install location, the build finds it automatically — via `PATH` on every OS, plus `/Applications` and Spotlight on macOS and `Program Files` on Windows. An explicit path set in the build file is always respected.
...
**Run scripts from the editor (Ctrl-B / Cmd-B)** — a build system runs the script you're editing directly in Praat, no copy-paste into Praat's script editor. The file is saved automatically before running. On Linux, Windows, and macOS 13 and earlier the default build sends to your open Praat and runs it, so dialogs, `form`/`beginPause`, the demo window, editors, and playback all work; on **macOS 14+** it opens the script in Praat's editor (press **Cmd-R** to run) because macOS 15 blocks the send-to-running mechanism. Praat is located automatically if it's not at the standard install path. Requires Praat 6.4.43+ (see Requirements). Adapted from Jörg Mayer's SublimePraat plugin.
...
- **macOS 14+ (Sonoma / Sequoia):** the script opens in Praat's script editor (press **Cmd-R** to run) rather than auto-running, because macOS 15 blocks the send-to-running-Praat Apple Event. No extra setup — just press Cmd-R after Ctrl-B / Cmd-B
...
**Build system:** adapted from Jörg Mayer's SublimePraat plugin ([praatpfanne.lingphon.net](https://praatpfanne.lingphon.net/praat-ressourcen/resources-english)), contributed at his suggestion. GPL-3.0-or-later.
